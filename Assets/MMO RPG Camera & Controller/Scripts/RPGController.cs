...
public class RPGController : MonoBehaviour {
	...
	private void Awake() {
		...
		try {
			...
			Debug.LogWarning(e.Message + ". Please set up all inputs needed for the RPGController according to the provided manual.");
			this.enabled = false;
		}
	}
	...
	private void Update() {
		...
		// Signal the motor to surface (swimming logic)
		_rpgMotor.Surface(Input.GetButton("Jump"));
		#endregion

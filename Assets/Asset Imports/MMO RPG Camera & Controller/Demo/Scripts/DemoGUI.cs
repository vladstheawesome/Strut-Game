...
public class DemoGUI : MonoBehaviour {
	...
	public Text _alignCharacter;
	public InputField _alignCharacterSpeed;
	public Toggle _alignCameraWhenMoving;
	public Toggle _supportWalkingBackwards;
	public InputField _alignCameraSmoothTime;
	public InputField _underwaterFogDensity;
	// RPGViewFrustum
	public Text _occlusionHandling;
	public Text _occludingLayers;
	...
	public InputField _swimSpeedMultiplier;
	...
	public InputField _swimmingStartHeight;
	public Toggle _diveOnlyWhenSwimmingForward;
	...
	public string[] OcclusionHandlings = {"DoNothing", "TagDependent", "AlwaysZoomIn"};
	public string[] AlignCharacterOptions = {"Never", "OnAlignmentInput", "Always"};
	public string[] OccludingLayersOptions = {"Nothing", "Everything", "Default", "Player"};
	...
	private void Awake() {		
		...
		if (character == null) {
			character = GameObject.Find("Character John");
		...
		_alignCharacter.text = RpgCamera.AlignCharacter.ToString();
		_alignCharacterSpeed.text = "" + RpgCamera.AlignCharacterSpeed;
		...
		_underwaterFogDensity.text = "" + RpgCamera.UnderwaterFogDensity;

		_occlusionHandling.text = RpgViewFrustum.OcclusionHandling.ToString(); // enum
		_occludingLayers.text = "Default";
		...
		_swimSpeedMultiplier.text = "" + RpgMotor.SwimSpeedMultiplier;
		...
		_swimmingStartHeight.text = "" + RpgMotor.SwimmingStartHeight;
		_diveOnlyWhenSwimmingForward.isOn = RpgMotor.DiveOnlyWhenSwimmingForward;
	...
	public void ValueChanged() {
		...
		RpgCamera.AlignCharacterSpeed = float.Parse(_alignCharacterSpeed.text);
		...
		RpgCamera.UnderwaterFogDensity = float.Parse(_underwaterFogDensity.text);
		...
		RpgMotor.SwimSpeedMultiplier = float.Parse(_swimSpeedMultiplier.text);
		...
		RpgMotor.SwimmingStartHeight = float.Parse(_swimmingStartHeight.text);
		RpgMotor.DiveOnlyWhenSwimmingForward = _diveOnlyWhenSwimmingForward.isOn;
	...
	public void ClickPreset1() {
		SceneManager.LoadScene("Preset 1");
	}

	public void ClickPreset2() {
		SceneManager.LoadScene("Preset 2");
	}

	public void ClickPreset3() {
		SceneManager.LoadScene("Preset 3");
	}

	public void ClickPreset4() {
		SceneManager.LoadScene("Preset 4");
	}
	...
	public void ClickOccultationHandling() {
		int index = Array.IndexOf(OcclusionHandlings, _occlusionHandling.text);
		index = (index + 1) % OcclusionHandlings.Length;
		RpgViewFrustum.OcclusionHandling = (OcclusionHandling)index;
		_occlusionHandling.text = OcclusionHandlings[index];
	}
	
	public void ClickOccultatingLayers() {
		int index = Array.IndexOf(OccludingLayersOptions, _occludingLayers.text);
		index = (index + 1) % OccludingLayersOptions.Length;
		if (OccludingLayersOptions[index] == "Nothing") {
			RpgViewFrustum.OccludingLayers = 0;
		} else if (OccludingLayersOptions[index] == "Everything") {
			RpgViewFrustum.OccludingLayers = -1;
		} else {
			RpgViewFrustum.OccludingLayers = 1 << LayerMask.NameToLayer(OccludingLayersOptions[index]);		
		}
		_occludingLayers.text = OccludingLayersOptions[index];
	}
	
	public void ClickAlignCharacter() {
		int index = Array.IndexOf(AlignCharacterOptions, _alignCharacter.text);
		index = (index + 1) % AlignCharacterOptions.Length;
		RpgCamera.AlignCharacter = (AlignCharacter)index;
		_alignCharacter.text = AlignCharacterOptions[index];
	}
	...
	public void ResetValues() {
		...
		_cameraPivotLocalPositionY.text = "" + 0.5;
		...
		_mouseYMin.text = "" + (-89.9);
		_mouseYMax.text = "" + 89.9;
		_mouseScrollSensitivity.text = "" + 15.0;
		_mouseSmoothTime.text = "" + 0.08;
		_minDistance.text = "" + 0;
		_maxDistance.text = "" + 20.0;
		_distanceSmoothTime.text = "" + 0.7;
		_alignCharacter.text = "OnAlignmentInput";
		RpgCamera.AlignCharacter = (AlignCharacter)Array.IndexOf(AlignCharacterOptions, _alignCharacter.text);
		_alignCharacterSpeed.text = "" + 10.0f;
		_alignCameraWhenMoving.isOn = true;
		_supportWalkingBackwards.isOn = true;
		_alignCameraSmoothTime.text = "" + 0.2;
		_underwaterFogDensity.text = "" + 0.06f;

		_occlusionHandling.text = "TagDependent";
		RpgViewFrustum.OcclusionHandling = (OcclusionHandling)Array.IndexOf(OcclusionHandlings, _occlusionHandling.text);
		_occludingLayers.text = "Default";
		RpgViewFrustum.OccludingLayers = 1;
